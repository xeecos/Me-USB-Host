...
#define	USB_MAX_DATA_LEN	0x40			/* max package length */
...
#define	CMD_RET_ABORT		0x5F			/* fail */
	...
	/* special interrupt state, only for CH372A/USBA */
...
#define	USB_INT_BUS_RESET4	0x0F			/* USB BUS RESET */
	...
	/* 2XH-3XH fail state response, only for USB/USBA */
	/*	 bit7~6 00 */
	/*	 bit5 1 */
	...
	/* 0x1X control state, only for USB/USBA */
...
class MeUsb
{
    public:
      ...
      bool device_online;
      bool device_ready;
      unsigned char RECV_BUFFER[ USB_MAX_DATA_LEN ];
      MeUsb();
      MeUsb(uint8_t s1, uint8_t s2);
      void init(char type);
      int initHIDDevice();
      int probeDevice();
      void resetBus();
    ...
    private:
      ...
      int stallCount;
      char usbtype;
      unsigned char USB_RD();
      void USB_WR(unsigned char c);
